...
def run():
    ...
    for e in ENZYMES:
        ...
        # Scientific notation, not %.4f: the active-site values are ~1e-12 %
        # and would print as +0.0000%, i.e. as the very thing under discussion.
        print(f"{e['name']:7} {e['partner']:18} {e['r']:8} {e['J']:<8.0f} {tau:8} "
              f"{e['T2e']:>4.0f}ns | {mfeE_coh:>+13.4e}% {mfeE_real:>+13.4e}% "
              f"{mfeO_real:>+13.4e}%")

    print("-" * 110)
    print("\nInterpretation:")
    print("  • MAO-A/B, DAO: active-site RP (r≈3.5 Å) ⇒ |J|≈0.75 GHz and τ≈10 ps")
    print("    (k=1e5 MHz). The 50 µT MFE is ~1.7e-12 %, capped by the purely")
    print("    kinetic ceiling (ω_B τ)² ≈ 7.8e-7 % that the picosecond lifetime")
    print("    imposes on its own — so the null holds in the coherent limit too.")
    print("  • CRY: separated RP (J≈0, τ≈µs) is the one magnetosensitive geometry,")
    print("    +1.0 % with T2e included. Same code, same numerics: what separates")
    print("    the two is inter-radical separation, not the flavin cofactor.")
    return results
...
def crossing_field_mT(J_MHz, D_MHz):
    """Field at which the S and T- levels of the active-site pair cross, in mT.

    With H = J S1.S2 + D (S1z S2z - S1.S2/3), the dipolar term shifts T+/T- by
    D/6 relative to the singlet, so the crossing is at hbar*omega_B = J + D/6 --
    NOT at J alone. Omitting D misplaces the resonance (750 MHz alone would give
    26.8 mT instead of 19.5 mT for monoamine oxidase).
    """
    from qbscreen.spin_dynamics import G_E, MU_B, HBAR
    return (J_MHz + D_MHz / 6.0) * 1e6 * TWO_PI * HBAR / (G_E * MU_B) * 1e3
