...
class TestQuantumZenoRobustness:
    ...
    def test_asymmetric_recombination_does_not_rescue_mao(self):
        ...
        def mfe_earth(kS, kT):
            def phi(B):
                H = TWO_PI * build_5spin_H(B, 14, 11.0, 44.0, 750.0, D_point_dipole(0.35))
                deph = electron_dephasing_ops(n, 1e-3, (0, 1))
                return singlet_yield_master(H, P_S, P_T, kS, kT, rho0, deph)  # 1/us, no 2pi
    ...
    def test_st_dephasing_does_not_open_a_window(self):
        ...
        def mfe(gamma_ST_MHz):
            def phi(B):
                ...
                return singlet_yield_master(H, P_S, P_T, 1e5, 1e5, rho0, deph)  # 1/us, no 2pi
            p0, pE = phi(0.0), phi(50e-6)
            return abs((pE - p0) / p0 * 100)
...
class TestReportedValueIsPhysical:
    """The 1.7e-12 % MAO number is small enough to be mistaken for a
    double-precision artefact of the 1024-dimensional solve. It is not: in the
    perturbative regime the field enters only through omega_B and time-reversal
    symmetry forbids a linear term, so the MFE must go as B^2. A numerical floor
    would show no such scaling. Backs ESI Table S1.
    """

    def test_mfe_scales_as_B_squared(self):
        import numpy as np, pytest
        from qbscreen.spin_dynamics import singlet_projector
        from qbscreen.master_equation import (singlet_yield_master,
                                              electron_dephasing_ops)
        from qbscreen.honest_mfe import build_5spin_H, D_point_dipole, TWO_PI

        n = 5
        P_S = singlet_projector(0, 1, n)
        P_T = np.eye(2 ** n) - P_S
        rho0 = P_S / np.trace(P_S)
        J, D, k = 750.0, D_point_dipole(0.35), 1e5      # MAO-A active site

        def phi(B_T, T2e_us):
            H = TWO_PI * build_5spin_H(B_T, 14, 11.0, 44.0, J, D)
            return singlet_yield_master(H, P_S, P_T, k, k, rho0,
                                        electron_dephasing_ops(n, T2e_us))

        for T2e_us in (None, 1e-3):                     # coherent, then T2e = 1 ns
            p0 = phi(0.0, T2e_us)
            # 100 uT ... 25 mT: below ~25 uT the double-precision floor intrudes.
            slopes = []
            for B_uT in (100, 500, 1000, 5000, 25000):
                mfe = (phi(B_uT * 1e-6, T2e_us) - p0) / p0 * 100
                slopes.append(mfe / B_uT ** 2)
            ref = slopes[0]
            for s in slopes:
                assert s == pytest.approx(ref, rel=3e-3), \
                    "MFE/B^2 must be constant; a numerical floor would not scale"

            # Extrapolating the converged slope to 50 uT must reproduce Table 1.
            expected = 1.6875e-12 if T2e_us is None else 1.6370e-12
            assert ref * 50 ** 2 == pytest.approx(expected, rel=0.02)


class TestCrossingField:
    """The S-T- crossing sets the paper's falsifiable prediction; it depends on
    the dipolar coupling, so it must be computed rather than taken as J alone."""

    def test_crossing_matches_numerical_peak(self):
        import numpy as np, pytest
        from qbscreen.honest_mfe import (build_5spin_H, TWO_PI, D_point_dipole,
                                         crossing_field_mT)
        from qbscreen.master_equation import singlet_yield_master
        from qbscreen.spin_dynamics import singlet_projector
        D = D_point_dipole(0.35)
        assert crossing_field_mT(750.0, D) == pytest.approx(19.5, abs=0.3)
        assert crossing_field_mT(400.0, D_point_dipole(0.40)) == pytest.approx(9.4, abs=0.3)
        # the analytic crossing must coincide with the numerical resonance
        n = 5; P_S = singlet_projector(0, 1, n); P_T = np.eye(2**n) - P_S
        rho0 = P_S / np.trace(P_S)
        def mfe(B):
            f = lambda b: singlet_yield_master(
                TWO_PI * build_5spin_H(b, 14, 11.0, 44.0, 750.0, D),
                P_S, P_T, 1.0, 1.0, rho0, [])
            p0 = f(0.0); return abs((f(B) - p0) / p0 * 100)
        scan = {B: mfe(B * 1e-3) for B in np.arange(17.0, 23.01, 0.5)}
        peak = max(scan, key=scan.get)
        assert abs(peak - 19.5) < 0.6, f"resonance at {peak} mT, expected ~19.5"
        # and the J-only value must NOT be the resonance
        assert mfe(26.8e-3) < 0.05 * scan[peak]
