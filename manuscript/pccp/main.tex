...
A detailed Data Availability Statement following RSC guidelines is supplied as a separate document with this submission (\texttt{data\_availability\_standalone.pdf}). In brief: the complete computational pipeline is released as the open-source Python package \texttt{qbscreen} (v0.1.0, MIT License) at \url{https://github.com/deeptell-inc/brain_protein_screening} and via \texttt{pip install qbscreen}; a versioned snapshot will be deposited on Zenodo with a citable DOI upon acceptance. All 9 PDB structures (publicly available from RCSB), computed JSON results, optimised geometries, HFC tensors, relaxation parameters, and an automated test suite (20 tests verifying every numerical claim) are archived in the repository.
